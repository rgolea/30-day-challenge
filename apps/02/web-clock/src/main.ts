...
function setDate() {
  ...
  rotateDegrees(minuteHand, calculateDegrees(minute, 'MINUTES') + second / 10);
  rotateDegrees(hourHand, calculateDegrees(hour, 'HOUR') + (minute / 2));
  if(timeRectangle) timeRectangle.innerHTML = `<span>${[hour, minute, second].map(formatter.format).join(':')}</span>`;
}
